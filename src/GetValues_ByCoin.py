...
for coin, balance in simple_balances['crypto'].items():
    ...
    crypto_data = crypto_globals.add_result_bycoin(crypto_data, coin, balance, float(json.loads(urllib.request.urlopen(url).read())[0]['price_' + display_ccy.lower()]))
...
for coin, balance in simple_balances['fiat'].items():
    crypto_data = crypto_globals.add_result_bycoin(crypto_data, coin, balance, 1 / fxrates['rates'][coin])
...
for coin, balance in simple_balances['iconomi_fund'].items():
    ...
    crypto_data = crypto_globals.add_result_bycoin(crypto_data, coin, balance, price_usd / usdrate)
...
crypto_globals.write_dictionary_as_json_file('crypto_values_bycoin.json', crypto_data)
