...
for address in api.get_balance_multiple():
    crypto_balances = crypto_functions.add_balance(crypto_balances, 'ETH',
...
for token in coin_config:
    ...
    if contract_address != None:
        ...
        for address in eth_config['addresses']:
            crypto_balances = crypto_functions.add_balance(crypto_balances, token,
...
for account in accounts.data:
    crypto_balances = crypto_functions.add_balance(crypto_balances, account['currency'], float(account['balance']['amount']),
...
for account in gdax_auth_client.get_accounts():
    crypto_balances = crypto_functions.add_balance(crypto_balances, account['currency'], float(account['balance']),
...
for account in bfx_auth_client.balances():
    crypto_balances = crypto_functions.add_balance(crypto_balances, account['currency'].upper(), float(account['amount']),
                                 'from Bitfinex')
...
for address in bch_config['addresses']:
    ...
    crypto_balances = crypto_functions.add_balance(crypto_balances, 'BCH', int(blockchair_result['data'][0]['value']) / coin_config['BCH'][
                                             'blockchair_units'],
                                 'from BCH address {addr}'.format(addr=address))

#Write data files
crypto_functions.write_dictionary_as_json_file('crypto_balances.json', crypto_balances)

# simplify balances from detail
simple_balances = crypto_functions.simplify_balances(coin_config, crypto_balances)
crypto_functions.write_dictionary_as_json_file('crypto_balances_aggregated.json', simple_balances)
