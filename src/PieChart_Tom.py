...
def generate_area_chart(files_to_process, multiplier=1.0, y_bottom=0.0):
    ...
    plt.set_title('Current value: £{:,.2f} ({} {:0.4f}% in 24h)               {}'.format(current_value,move_direction,move*100,df.iloc[-1:].index[0].strftime('%Y-%m-%d %H:%M:%S')),fontdict ={'fontsize':18})
